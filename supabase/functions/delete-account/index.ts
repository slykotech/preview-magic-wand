...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1. Delete game-related data first
    ...
    // 2. Delete AI coach messages first
    ...
    // 4. Delete story responses and views for user's stories
    ...
    // 5. Delete user's own story responses and views
    await safeDelete('story_responses', async () => {
    ...
    // 10. Delete memory images for user's memories
    ...
    // 16. Delete user conversation clears
    ...
    // 17. Delete subscription-related data
    ...
    // 18. Get couple IDs where user is involved
    const { data: userCouples } = await supabaseAdmin
      .from('couples')
      .select('id')
      .or(`user1_id.eq.${userId},user2_id.eq.${userId}`)

    // 19. Delete couple-related data
    ...
    // 22. Delete pending verifications
    if (user.email) {
    ...
    // 24. Finally, delete the user from auth.users
