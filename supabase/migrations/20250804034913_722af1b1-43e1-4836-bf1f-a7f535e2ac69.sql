-- Fix the sync_scores and historical_sync_scores tables to have proper unique constraints

-- First, check if sync_scores table has the right constraint
ALTER TABLE public.sync_scores DROP CONSTRAINT IF EXISTS sync_scores_couple_id_calculated_date_key;
ALTER TABLE public.sync_scores ADD CONSTRAINT sync_scores_couple_id_calculated_date_key UNIQUE (couple_id, calculated_date);

-- Same for historical_sync_scores
ALTER TABLE public.historical_sync_scores DROP CONSTRAINT IF EXISTS historical_sync_scores_couple_id_calculated_date_key;
ALTER TABLE public.historical_sync_scores ADD CONSTRAINT historical_sync_scores_couple_id_calculated_date_key UNIQUE (couple_id, calculated_date);

-- Update the calculate_enhanced_sync_score function to handle the constraint properly
...
BEGIN
  ...
  -- Count days where both partners checked in (+3 points each)
  ...
  -- Count days where only one partner checked in (-2 points)
  ...
  -- Count days where both missed check-in (-3 points each)
  ...
  checkin_points := GREATEST(checkin_points, -21);
  checkin_points := LEAST(checkin_points, 21);
  
  -- Calculate story sharing (last 7 days)
  -- Count days where both partners shared stories (+3 points each)
  ...
  -- Count days where only one partner shared story (-2 points)
  ...
  -- Count days where both missed stories (-3 points each)
  WITH all_recent_days AS (
    SELECT generate_series(
      CURRENT_DATE - INTERVAL '7 days',
      CURRENT_DATE - INTERVAL '1 day',
  ...
  story_points := GREATEST(story_points, -21);
  story_points := LEAST(story_points, 21);
  ...
  story_points := story_points + LEAST(story_interactions, 8);
  
  -- Calculate communication quality (last 7 days)
  ...
  -- Communication scoring: 1 point per message, max 30
  communication_points := LEAST(recent_messages, 30);
  
  -- Calculate relationship milestones (last 7 days)
  ...
  -- Memory: +8 each, Completed dates: +15 each, max 25 total
  ...
  IF previous_checkin_streak > current_checkin_streak AND previous_checkin_streak > 0 THEN
    streak_penalty := streak_penalty + LEAST(previous_checkin_streak, 10);
  END IF;
  
  IF previous_story_streak > current_story_streak AND previous_story_streak > 0 THEN
    streak_penalty := streak_penalty + LEAST(previous_story_streak, 10);
  END IF;
  ...
  -- Store detailed breakdown in sync_scores table with proper upsert
  ...
  -- Store in historical scores with proper upsert
  INSERT INTO public.historical_sync_scores (
    ...
    streak_bonus,
    jsonb_build_object(
      'calculation_details', 'Enhanced algorithm v4.1 - Fixed constraints',
