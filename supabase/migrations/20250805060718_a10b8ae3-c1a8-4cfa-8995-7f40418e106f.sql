...
-- Update couples table with proper constraints
...
-- Add proper unique constraint and check constraint
ALTER TABLE public.couples 
ADD CONSTRAINT unique_couple UNIQUE(user1_id, user2_id),
ADD CONSTRAINT different_users CHECK (user1_id != user2_id);
...
-- Function to prevent multiple connections
CREATE OR REPLACE FUNCTION public.check_single_partner_rule()
RETURNS TRIGGER AS $$
BEGIN
  -- Check if user1_id is already in another couple
  IF EXISTS (
  ...
  -- Check if user2_id is already in another couple (unless it's a demo connection)
  IF NEW.user1_id != NEW.user2_id AND EXISTS (
    SELECT 1 FROM public.couples 
