      ...
      <div className="max-w-4xl mx-auto">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mt-4 p-4 bg-gray-900 text-white rounded-lg font-mono text-xs">
            <div className="font-bold text-yellow-300 mb-2">🚨 DEBUG: Card Distribution Issue</div>
            <button
              onClick={async () => {
                console.group('🔍 EMERGENCY CARD DEBUG');
                ...
                console.log('📊 Database cards:', distribution);
                
                // Check played cards for this session  
                ...
                // Get card types for responses
                if (responses && responses.length > 0) {
                  const cardIds = responses.map(r => r.card_id);
                  const { data: responseCards } = await supabase
                    .from("deck_cards")
                    .select("id, response_type")
                    .in("id", cardIds);
                  
                  const responseDistribution = responseCards?.reduce((acc, card) => {
                    acc[card.response_type] = (acc[card.response_type] || 0) + 1;
                    return acc;
                  }, {} as Record<string, number>);
            ...
            <button
              onClick={async () => {
                // Force next card to be photo or text
                const { data: nonActionCards } = await supabase
                  .from("deck_cards")
                  .select("*")
                  .eq("is_active", true)
                  .not("response_type", "eq", "action")
                  .limit(1);
                
                if (nonActionCards && nonActionCards.length > 0) {
                  await supabase
                    .from("card_deck_game_sessions")
                    .update({
                      current_card_id: nonActionCards[0].id,
                      current_card_revealed: false,
                      last_activity_at: new Date().toISOString()
                    })
                    .eq("id", sessionId);
                  
                  alert(`Forced ${nonActionCards[0].response_type} card!`);
                  window.location.reload();
                } else {
                  alert('No non-action cards found!');
                }
              }}
              className="px-3 py-1 bg-red-600 text-white rounded"
            >
              Force Non-Action Card
            </button>
          </div>
        )}
