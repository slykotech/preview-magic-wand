...
export const Games = () => {
  ...
  const fetchGames = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('card_games').select('*').eq('is_active', true).order('name');
  ...
  const getGameIcon = (gameType: string) => {
    switch (gameType) {
      case 'identity_dreams':
        return <Sparkles className="w-8 h-8" />;
      case 'love_language_lab':
  ...
  if (loading) {
    return <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 pb-20">
      <GradientHeader title="Relationship Games" subtitle="Strengthen your connection through playful interaction" icon={<GamepadIcon className="w-6 h-6" />} />
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 pb-20">
      ...
      <div className="max-w-md mx-auto p-4 space-y-6">
        ...
        <div className="space-y-4">
          {games.map(game => <Card key={game.id} className="group hover-scale transition-all duration-300 hover:shadow-lg">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant="outline" className={getDifficultyColor(game.difficulty_level)}>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button onClick={() => startGame(game.id)} className="group-hover:scale-105 transition-transform duration-200" disabled={!coupleData?.id}>
        ...
        {games.length === 0 && <Card className="text-center py-8">
            <CardContent>
